...
- **Friction from the WoW itself:** Two things, one of them structural.
  ...
  **(2) Nothing protects a check from being vacuous.** Five defects of one family appeared in this feature alone — the fifth caught while writing the fix for the fourth, an assertion pointing at a fixture from another block so it silently checked a missing file and recorded neither PASS nor FAIL: a self-scanning check that matched its own grep line; two assertions that passed because the searched word already existed elsewhere in the file; and an unanchored pattern (`rails` matching "guardrails"). `check_90` was bitten by the same family in `e6bc658`. Every one was caught by hand, and the worst of them — a `Guard` that validated and never ran — was caught only because a human asked a question the artifacts could not answer. The harness has extensive machinery to ensure tests exist and go red, and none to ensure an assertion can actually fail. Also minor: the `/tasks` gate had to be walked with hand-written `awk`; `status.sh` reports phases but does not execute that gate.
