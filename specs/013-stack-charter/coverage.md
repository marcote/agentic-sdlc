...
|---|---|---|---|---|---|---|
| `measurable-impact` · `real-enforcement` | O1 no assumption stays mute | `memory/stack/` + `/stack` command & skill, positioned in the loop | STACK-CMD | project | `check_92_stack.sh` | ✅ uat |
...
| `measurable-impact` | O1 no assumption stays mute | S0 is the first pin, blast-radius-derived, carries a Falsifier | S0-PIN | project | `check_92_stack.sh` | ✅ uat |
| `real-enforcement` | O1 · O4 rigor without softening the rules | S0 scales scope only; RED gate, 100% coverage and the P6 floor do not scale | S0-SCOPE-ONLY | project | `check_92_stack.sh` | ✅ uat |
| `agnostic-portability` | O4 no stack prescribed | No tool/language/runtime/vendor named as a required default in `memory/stack/base/` | NO-PRESCRIBE | project | `check_92_stack.sh` | ✅ uat |
| `real-enforcement` | O3 hedge is verified, not written | `/distill` step 1 injects [stance] pins' Injects rows | DISTILL-STANCE | project | `check_92_stack.sh` | ✅ uat |
| `agnostic-portability` · `frictionless-adoption` | O4 the mechanism vendors, the pins do not | `vendor.sh`: `memory/stack/base/` KEEP, `stack.md` SEED, CLAUDE.md `## Stack` points at charter | VENDOR-STACK | project | `check_92_stack.sh` | ✅ uat |
| `measurable-impact` | O5 charter health backed by evidence | `wow-report` emits pins-tripped vs rework-with-no-pin | WOW-HEALTH | project | `check_92_stack.sh` | ✅ uat |
| `real-enforcement` · `measurable-impact` | O1 (D3 reflexive dogfood) | The harness's own live decisions are pinned (py3 engine, bash-only, no-runtime) | CHARTER-SEED | project | `check_92_stack.sh` | ✅ uat |
| — | — | Re-running `/stack` does not duplicate or drop pins; SUPERSEDED preserved | RERUN-IDEMPOTENT | `[given] base/idempotency` | `check_92_stack.sh` | ✅ uat |
| — | — | Each amended pin records date + reason + what tripped it | AMEND-TRAIL | `[given] base/audit-logging` | `check_92_stack.sh` | ✅ uat |
| `frictionless-adoption` · `real-enforcement` | O1 no assumption stays mute (adopter day one) | Empty charter reported distinctly from malformed; `/plan` refuses on empty as on absent | EMPTY-CHARTER | project · **found at `/uat`** | `check_92_stack.sh` | ✅ uat |
| — | — | Suite builds own fixtures; passes detached-HEAD, no-TTY | HERMETIC-ENV | `[given] base/hermetic-tests` | `check_92_stack.sh` | ✅ uat |
...
**UAT (2026-08-08) — one product gap found, routed to `/distill`, closed.**

UAT validated against the brief's objective by vendoring onto a clean repo and inspecting what
an adopter actually receives. That surfaced a state no criterion covered: **vendoring never
produces "no charter"** — it seeds a stub, so an adopter's day-one state is a well-formed file
with zero pins. The engine reported it as `malformed` (exit 2) and `/plan`'s gate only handled
an *absent* charter. A fresh adopter's first interaction with the feature was an error message
about a bug that did not exist.

Routed as a **product gap** (a missing criterion, not a failing one) back through `/distill`,
per the UAT checklist. `EMPTY-CHARTER` was added to `spec.md` and `acceptance.md`, **proved
🔴 RED** (4 failing assertions; the fifth — an unreadable charter still exiting 2 — passed as
the control that keeps the distinction meaningful), then fixed in `scripts/stack/engine.py`
(exit 3 = empty, `guards`/`exposure` succeed on it) and `.claude/commands/plan.md` (absent and
empty are one refusal, deliberately). Suite: 315/4 at RED → **320 PASS / 0 FAIL**.

*A second vacuous assertion was caught in the same pass:* `plan.md =~ /empty/` passed because
the word already appeared in an unrelated sentence. Replaced with assertions on the actual
contract (`absent … or empty`, `zero pins`). Second occurrence of this shape in one feature.

**19 deterministic criteria ✅ uat · coverage 100%.**

**The three `📋 case` rows stay open, deliberately.** Closing them would mean judging my own
output, which is not evidence. They become a `pending-observation` for `/retro`, with an
