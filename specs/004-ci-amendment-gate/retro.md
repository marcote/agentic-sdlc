...
- **Gaps caught by /distill:** 6 edge cases + 1 reframe `[deriv: spec.md "Edge cases (80% problem)" 6 bullets @ 3d13431; 12 criteria in coverage.md]` — the key ones: set-semantics (reformat ≠ change); schema-invalid even with an ADR; prose and threshold need no ADR; editing an existing ADR is not `hasAdrFor`. Plus the DEP-FREE reframe toward "bash+python3 stdlib" @ 7c3ad73: python3 is a system interpreter, not an installable dep.
...
- **Friction from the WoW itself:** the state machine `no contract → red → green` assumes every deterministic criterion has a RED→GREEN arc. An **invariant** criterion does not. DEP-FREE had to be tied to a deliverable so the `/tasks` gate would accept it. Separately, *GitHub config* criteria (AMEND-BLOCK-REAL/PUSH) are not hermetically unit-testable. They needed a real outward-facing walk, which `tests/run.sh` does not validate.
