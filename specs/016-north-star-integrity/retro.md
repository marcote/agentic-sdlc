...
- **Friction from the WoW itself:**
  **(1) Two of this feature's own assertions were vacuous, and `nvc.sh` caught neither.** `ALIGN-STAMPS-PROVENANCE` grepped for the word `since` — an ordinary English word that appeared in a scoring rationale — so it passed against an `alignment.md` containing no stamp at all. `AMEND-PROV-ONLY` cannot reach the code it claimed to test: a provenance-only edit leaves the governed sets unchanged, so the gate short-circuits at `sets-changed`; proved by mutation, which left it green while breaking three unrelated criteria. **This is the second consecutive feature where the mechanical half missed the vacuity and reading caught it.** Both are semantic vacuity, which `base/patterns/non-vacuous-checks.md` explicitly places out of mechanical scope — so the pattern is *accurate*, and the cost of that accuracy is now measured at two occurrences in two features.
...
- **Candidate rules → constitution:** none. The obvious candidate — *"an assertion must not match an ordinary word"* — is **semantic vacuity by definition**, which the pattern already declares out of mechanical scope and which two features have now confirmed cannot be automated cheaply. Writing it as prose would be the third proposal of a rule this repository has measured as prose-resistant. It goes to `docs/backlog.md` instead, with the two occurrences as its evidence.
