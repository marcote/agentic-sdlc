...
- **Gaps caught by /distill:** 3 grilling ambiguities (KEEP-collision policy, test-cmd landing, provenance/git), plus edge cases (re-run idempotency, unknown stack, non-git source, dry-run read-only, DROP leakage), plus one classification refinement: `tests/` moved KEEP→DROP `[deriv: spec.md "Edge cases" + "Open questions" + coverage.md; git b85c912]`. The notable one is that last: `tests/` validates the harness-as-product, so it is DROP.
...
- **Friction from the WoW itself:** (1) the `DEPFREE` grep, copy-pasted from check_82/95, is too naive for a script that legitimately *names* toolchains as data. It belongs in `tests/lib.sh` as a shared, invocation-aware helper, not duplicated per check. (2) The pipeline tracklist is still inferred by hand (feature 008, pending). (3) Positive: `/align` finally ran on the real engine. The bootstrap friction 006's retro flagged is gone.
