...
# _skip LABEL: reason -- a criterion that genuinely cannot execute HERE emits this instead of
# nothing. Silence is not a valid outcome: from the outside a skipped criterion and a broken
# one look identical, which is the whole defect nvc.sh exists to catch. Not counted as a pass.
_skip(){ echo "  SKIP: $1"; }
